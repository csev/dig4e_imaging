...
| **`dig4e_imaging`** (this repo) | https://image.dig4e.com | Imaging course (Tsugi/Koseu) |
| `dig4e_audio` | https://audio.dig4e.com | Audio course (Tsugi/Koseu) |
| `dig4e_video` | https://video.dig4e.com | Video course (Tsugi/Koseu) |

All four are **sibling git repos** checked out next to one another (see `dig4e-www/dig4e.code-workspace` for a four-root Cursor/VS Code workspace). When work touches hub navigation, shared branding, or cross-site URLs, check `dig4e-www`. For parallel patterns in the other courses, compare `dig4e_audio` and `dig4e_video`.

## What this repo is

The Imaging module of Dig4E — an online course on standards and best practices for creating archival-quality digital still images of cultural heritage materials.
...
- Lesson styling: `css/lessons.css` (imaging-specific Tsugi class names)
- Local Tsugi settings: `tsugi_settings.php`
- LTI tools: `tools/`
- Shared images hosted at `https://image.dig4e.com/images/` (used by sibling sites too)

## Conventions

- Preserve `lessons.json` module structure when editing course content
- Brand accent color: `#9e4f00`
- Match patterns used in the audio and video siblings where the sites are intentionally parallel
